...
var (
	...
	cacheInquoraLikeLikeRecordIdPrefix               = "cache:inquoraLike:likeRecord:id:"
	cacheInquoraLikeLikeRecordBizIdObjIdUserIdPrefix = "cache:inquoraLike:likeRecord:bizId:objId:userId:"
)
...
func (m *defaultLikeRecordModel) Delete(ctx context.Context, id int64) error {
	...
	inquoraLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheInquoraLikeLikeRecordBizIdObjIdUserIdPrefix, data.BizId, data.ObjId, data.UserId)
	inquoraLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheInquoraLikeLikeRecordIdPrefix, id)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, inquoraLikeLikeRecordBizIdObjIdUserIdKey, inquoraLikeLikeRecordIdKey)
	return err
}
...
func (m *defaultLikeRecordModel) FindOne(ctx context.Context, id int64) (*LikeRecord, error) {
	inquoraLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheInquoraLikeLikeRecordIdPrefix, id)
	var resp LikeRecord
	err := m.QueryRowCtx(ctx, &resp, inquoraLikeLikeRecordIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultLikeRecordModel) FindOneByBizIdObjIdUserId(ctx context.Context, bizId string, objId int64, userId int64) (*LikeRecord, error) {
	inquoraLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheInquoraLikeLikeRecordBizIdObjIdUserIdPrefix, bizId, objId, userId)
	var resp LikeRecord
	err := m.QueryRowIndexCtx(ctx, &resp, inquoraLikeLikeRecordBizIdObjIdUserIdKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultLikeRecordModel) Insert(ctx context.Context, data *LikeRecord) (sql.Result, error) {
	inquoraLikeLikeRecordBizIdObjIdUserIdKey := fmt.Sprintf("%s%v:%v:%v", cacheInquoraLikeLikeRecordBizIdObjIdUserIdPrefix, data.BizId, data.ObjId, data.UserId)
	inquoraLikeLikeRecordIdKey := fmt.Sprintf("%s%v", cacheInquoraLikeLikeRecordIdPrefix, data.Id)
...
func (m *defaultLikeRecordModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheInquoraLikeLikeRecordIdPrefix, primary)
}
